    ...
    public static class PersonScannerAdapter implements PersonScanner{
        ...
        public Person read() throws IOException {
            ...
            Date simpleDateFormat = null;
                s = fileScanner.nextLine().split("\\s+");
                String s2 = s[3] + "/" + s[4] + "/" + s[5];
            try {
                 simpleDateFormat =  new SimpleDateFormat("dd/MM/yyyy", Locale.ENGLISH).parse(s2);
            } catch ( ParseException e ) {
                e.printStackTrace();
            }
            Person person = new Person(s[1], s[2],s[0], simpleDateFormat);
            return person;
        }
